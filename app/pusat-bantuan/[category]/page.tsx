...
export function generateMetadata({ params }: { params: { category: string } }): Metadata {
  const data = helpCategories[params.category as keyof typeof helpCategories]
  if (!data) {
    ...
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-[#f5fbfb] to-white py-28 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-[#0B4B50] mb-2">
            {data.title}
          </h1>
          ...
          <div className="space-y-4">
            {data.items.map((item) => (
              <Link
                ...
                className="group flex items-center justify-between p-5 rounded-2xl border border-[#d7ecec] bg-white hover:border-[#0FA3A8] hover:shadow-md transition-all duration-200"
              >
                <div>
                ...
                <ChevronRight size={22} className="text-[#0FA3A8] opacity-70 group-hover:opacity-100" />
              </Link>
            ))}
          </div>
        </div>
