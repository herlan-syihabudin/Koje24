...
export default function PackagesSection() {
  ...
  return (
    <section id="paket" className="bg-white py-16 md:py-24 px-6 md:px-14 lg:px-24">
      <div className="text-center mb-10">
        <h2 className="font-playfair text-3xl md:text-4xl font-semibold mb-3 text-[#0B4B50]">
          Paket Hemat KOJE24
        </h2>
        <p className="text-gray-600 max-w-2xl mx-auto">
          Pilih paket siap minum. Checkout langsung via WhatsApp tanpa masuk keranjang.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {plans.map(p => (
          <div key={p.id} className="border border-[#e6eeee] rounded-2xl p-6 bg-[#f9fbfb]">
            <h3 className="text-lg font-semibold text-[#0B4B50] mb-1">{p.name}</h3>
            <p className="text-sm text-gray-600 mb-4">{p.desc}</p>
            <div className="text-2xl font-bold text-[#0FA3A8] mb-4">Rp{p.price.toLocaleString("id-ID")}</div>
            <button
