...
export default function HistoryOrdersPage() {
  ...
  useEffect(() => {
    fetch("/api/dashboard/orders?status=SELESAI")
      .then(res => res.json())
      .then(data => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-semibold">Riwayat Order</h1>
          <p className="text-sm text-gray-600 mt-1">
            Order yang sudah selesai ({orders.length} order)
          </p>
        </div>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-2xl border p-12 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-600">Belum ada riwayat order</h2>
          <p className="text-sm text-gray-400 mt-2">Order yang selesai akan muncul di sini</p>
        </div>
      ) : (
